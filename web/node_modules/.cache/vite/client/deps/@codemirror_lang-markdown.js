import {
  ...
  ifNotIn,
  snippetCompletion
} from "./chunk-3QE6RK6F.js";
import {
  ContextTracker,
  ExternalTokenizer,
  LRParser,
  LocalTokenGroup
} from "./chunk-THXBMSNL.js";
import {
  IterMode,
  NodeProp,
  NodeSet,
...
function localCompletionSource(context) {
  let inner = syntaxTree2(context.state).resolveInner(context.pos, -1);
...
var javascriptLanguage = LRLanguage2.define({
  name: "javascript",
  parser: parser4.configure({
    props: [
      indentNodeProp2.add({
        IfStatement: continuedIndent2({ except: /^\s*({|else\b)/ }),
        TryStatement: continuedIndent2({ except: /^\s*({|catch\b|finally\b)/ }),
        ...
        "Statement Property": continuedIndent2({ except: /^\s*{/ }),
        JSXElement(context) {
...
function javascript(config = {}) {
  ...
  return new LanguageSupport2(lang, [
...
var autoCloseTags = EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    var _a;
    let { head } = range, around = syntaxTree2(state).resolveInner(head - 1, -1), name;
...
function htmlCompletionFor(schema, context) {
  let { state, pos } = context, tree = syntaxTree3(state).resolveInner(pos, -1), around = tree.resolve(pos);
...
var htmlPlain = LRLanguage3.define({
  name: "html",
...
var autoCloseTags2 = EditorView2.inputHandler.of((view, from, to, text, insertTransaction) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    let { head } = range, after = syntaxTree3(state).resolveInner(head, -1), name;
    if (didType && text == ">" && after.name == "EndTag") {
      ...
      if (after.from == head - 2 && ((_c = tag.lastChild) === null || _c === void 0 ? void 0 : _c.name) != "CloseTag" && (name = elementName2(state.doc, tag, head)) && !selfClosers2.has(name)) {
        ...
        let insert = `${name}>`;
        return {
          range: EditorSelection2.cursor(head + insert.length, -1),
...
// node_modules/@codemirror/lang-markdown/dist/index.js
var data = defineLanguageFacet2({ commentTokens: { block: { open: "<!--", close: "-->" } } });
var headingProp = new NodeProp();
var commonmark = parser.configure({
  props: [
    foldNodeProp4.add((type) => {
...
var headerIndent = foldService.of((state, start, end) => {
  for (let node = syntaxTree4(state).resolveInner(end, -1); node; node = node.parent) {
    if (node.from < start)
      break;
...
var insertNewlineContinueMarkupCommand = (config = {}) => ({ state, dispatch }) => {
  let tree = syntaxTree4(state), { doc } = state;
  let dont = null, changes = state.changeByRange((range) => {
    ...
    if (inner.item && emptyLine) {
      ...
      if (first.to >= pos || second && second.to < pos || line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text) || config.nonTightLists === false) {
        ...
        return { range: EditorSelection3.cursor(delTo + insert2.length), changes: changes3 };
      } else {
        let insert2 = blankLine(context, state, line);
        return {
          range: EditorSelection3.cursor(pos + insert2.length + 1),
    ...
    return { range: EditorSelection3.cursor(from + insert.length + 1), changes: changes2 };
  });
  if (dont)
    return false;
    ...
    if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
      ...
      if (context.length) {
        ...
        if (pos - line.from > spaceEnd && !/\S/.test(line.text.slice(spaceEnd, pos - line.from)))
          return {
            range: EditorSelection3.cursor(line.from + spaceEnd),
        ...
        (!inner.item || line.from <= inner.item.from || !/\S/.test(line.text.slice(0, inner.to)))) {
          ...
          if (inner.item && inner.node.from < inner.item.from && /\S/.test(line.text.slice(inner.from, inner.to))) {
            ...
            if (start == line.from)
              insert = normalizeIndent(insert, state);
            return {
              range: EditorSelection3.cursor(start + insert.length),
              changes: { from: start, to: line.from + inner.to, insert }
            };
          }
          if (start < pos)
            return { range: EditorSelection3.cursor(start), changes: { from: start, to: pos } };
        }
      }
    }
    return dont = { range };
...
function markdown(config = {}) {
  ...
  if (defaultCodeLanguage instanceof LanguageSupport4) {
  ...
  return new LanguageSupport4(lang, support);
}
function htmlTagCompletion(context) {
  let { state, pos } = context, m = /<[:\-\.\w\u00b7-\uffff]*$/.exec(state.sliceDoc(pos - 25, pos));
  if (!m)
    return null;
  let tree = syntaxTree4(state).resolveInner(pos, -1);
...
var pasteURLAsLink = EditorView3.domEventHandlers({
  paste: (event, view) => {
    var _a;
    ...
    if (!markdownLanguage.isActiveAt(view.state, main.from, 1))
      return false;
    let tree = syntaxTree4(view.state), crossesNode = false;
    tree.iterate({
      from: main.from,
      to: main.to,
      enter: (node) => {
