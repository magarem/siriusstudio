...
const collectionPanelVisible = ref(true)
// --- MODAIS ---
...
const { data: rawFileContent } = await useAsyncData(
  ...
  () => {
    ...
    const folder = currentFile.value.substring(
      0,
      currentFile.value.lastIndexOf("/"),
    );
...
watch(rawFileContent, (newVal) => {
  if (newVal?.content) {
    ...
    fmSchema.value = parsed.isRaw
      ? "none"
      : parsed.frontmatter?.schema || "default";

    console.log(
      `Arquivo carregado. RAW: ${parsed.isRaw}. Schema: ${fmSchema.value}`,
    );
  } else {
...
const handleNavigate = {
  enterCollectionDir: async (f) => {
    const aa  = `${currentFolder.value}/${f}`;
    await getFolderContent(aa)
  },
  ...
  selectFile: async (path) => {
    ...
    if (fullPath.endsWith("/_index.md") || fullPath.endsWith("/index.md")) {
      try {
        ...
        if (altIndex) {
          console.log(
            `[Redirecionamento] _index.md não encontrado. Abrindo: ${altIndex}`,
          );
          fullPath = altIndex;
        }
      }
...
//     }else{
    ...
    if (isIndexFile) {
      try {
        ...
        const folderObj = parentItems.find(
          (f) => f.name === folderName && f.isDirectory,
        );

        if (folderObj && !folderObj.hasChildren) {
          
          
          if (!isCollectionFolder) currentFolder.value = parentFolder;
    ...
    if (fileFolder !== currentFolder.value) {
      if (!isCollectionFolder) currentFolder.value = fileFolder;
    }
  },
};
...
const getFolderContent = async (folder) => {
  try {
    const files = await $fetch("/api/admin/storage", {
      params: {
        site: siteContext.value,
        folder: folder
      },
    ...
    // 1. Atualiza a lista de arquivos
    currentFolderFiles.value = files || [];

    // 2. VERIFICAÇÃO: Procura se existe algum arquivo com nome '.collection'
    if (Array.isArray(files)) {
        isCollectionFolder.value = files.some(file => file.name === '.collection');
    } else {
        isCollectionFolder.value = false;
    }

    console.log(`Pasta: ${folder} | É Coleção: ${isCollectionFolder.value}`);
...
const handlePreview = () => {
  ...
  cleanPath = cleanPath
    .replace(/\/_index\.(md|toml|yaml|yml)$/, "")
    .replace(/\/_index$/, "");
  cleanPath = cleanPath
    .replace(/\/index\.(md|toml|yaml|yml)$/, "")
    .replace(/\/index$/, "");
...
const saveFile = async () => {
  ...
  try {
    ...
    if (isRawFile.value) {
      ...
      const yamlPart = yaml.dump(fileData.value.frontmatter, {
        indent: 2,
        lineWidth: -1,
        noRefs: true,
      });
...
const handlePublish = async () => {
  ...
  toast.add({
    severity: "info",
    summary: "Publicando...",
    detail: "Gerando arquivos estáticos...",
    life: 2000,
  });
  try {
...
const editorCtxFolder = computed(() => {
  ...
  return lastSlash !== -1
    ? currentFile.value.substring(0, lastSlash)
    : "content";
});
...
// Função para tratar o clique na linha da tabela
const openCollectionItem = (item) => {
    // Monta o caminho relativo
    let targetPath = `${activeCollection.value}/${item.name}`;
    
    // Se o item for um diretório (ex: "bourbon"), o arquivo real é o _index.md dentro dele
    if (item.isDirectory) {
        targetPath += '/_index.md';
    }
    
    console.log("Abrindo item:", targetPath);
    handleNavigate.selectFile(targetPath);
};
 
const loadFile = (item) => {
 collectionPanelVisible.value = false
 editArea.value = true
 handleNavigate.selectFile(item.path.replace('_index.md', '') + '/_index.md')
}
...
<template>
  ...
  >
    <header
      class="h-14 bg-[#141b18] border-b border-white/5 shrink-0 flex items-center justify-between px-4 z-20 select-none shadow-sm relative overflow-hidden"
    >
      <div class="flex items-center">
        <div class="flex items-center gap-2 cursor-default group mr-4">
          <i
            class="pi pi-star-fill text-[#6f942e] text-sm group-hover:scale-110 transition-transform duration-300 drop-shadow-[0_0_6px_rgba(111,148,46,0.5)]"
          ></i>
          <div _class="flex flex-col leading-none">
            <span
              class="font-black text-slate-200 text-lg tracking-tight group-hover:text-white transition-colors"
              >Sirius</span
            >
            <span
              class="text-[14px] text-[#6f942e]/80 font-bold group-hover:text-[#6f942e] transition-colors pl-2"
              >Studio</span
            >
          </div>
        </div>
        ...
        >
          <div
            class="w-1.5 h-1.5 rounded-full bg-[#6f942e] animate-pulse shadow-[0_0_6px_#6f942e]"
          ></div>
          <span
            class="text-xs font-mono text-slate-300 font-medium tracking-tight group-hover/ctx:text-white transition-colors"
          >
            {{ siteContext }}
          </span>
        </div>
        ...
        <div class="relative">
          ...
          >
            <div class="hidden md:flex flex-col items-end leading-none">
              <span
                class="text-[11px] font-bold text-slate-300 group-hover:text-white"
                >Admin</span
              >
            </div>
            <div
              class="w-8 h-8 rounded-full bg-gradient-to-b from-zinc-700 to-zinc-800 border border-white/10 flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform"
            >
    ...
    <div class="flex-1 flex flex-row overflow-hidden relative">
      ...
      >
        <button
          ...
          class="w-8 h-8 rounded-md flex items-center justify-center transition-all duration-200 group relative"
          :class="
            showFileManager
              ? 'bg-[#6f942e]/10 text-[#6f942e]'
        ...
        >
          ...
          <div
            ...
            class="absolute left-0 top-2 bottom-2 w-[2px] bg-[#6f942e] rounded-r-full"
          ></div>
        </button>
      ...
      >
        <Sidebar
          ...
          @enterCollectionDir="handleNavigate.enterCollectionDir"
          @navigate="handleNavigate.enterFolder"
          @navigate_intoCollectionDir="handleNavigate.enterCollectionDir"
      ...
      <main class="flex-1 flex flex-col min-w-0 bg-[#0a0f0d] relative">
        <slot name="workspace-content">
          <div
            v-if="currentFile"
            class="h-12 border-b border-white/5 flex items-center justify-between px-4 bg-[#0a0f0d] shrink-0"
          >
            <div
              class="flex items-center gap-2 text-xs font-mono text-slate-400 overflow-hidden"
            >
            ...
            <div
              class="flex items-center bg-white/5 rounded-lg p-1 border border-white/5 gap-1"
            >
              <button
                @click="saveFile"
              ...
              >
                <i
                  class="pi"
                  :class="
                    loadingPublish ? 'pi-spin pi-spinner' : 'pi-cloud-upload'
                  "
                ></i>
          ...
          <div
            v-if="currentFile"
            class="flex-1 flex flex-row overflow-hidden relative"
          >
            <div
              class="flex-1 flex flex-col bg-[#0a0f0d] min-w-0 h-full relative"
            >
              <CollectionFiles
                v-if="isCollectionFolder && collectionPanelVisible"
                :files="currentFolderFiles"
                @select="loadFile"
              />
              <AdminMarkdownEditor
                v-if="!collectionPanelVisible || editArea"
            ...
            <div
              v-if="(!isRawFile && !collectionPanelVisible) && fields.length > 0"
          ...
          <div
            v-else
            class="flex-1 flex items-center justify-center flex-col opacity-20 select-none"
          >
    ...
    <footer
      class="h-6 bg-[#141b18] border-t border-white/5 shrink-0 flex items-center justify-between px-3 text-[10px] text-slate-500 z-20 font-mono"
    ></footer>
    ...
    >
      <div class="h-[80vh] bg-[#141b18] flex flex-col">
        <div
          class="p-4 border-b border-white/5 flex justify-between items-center bg-[#111614]"
        >
          <span class="font-bold text-slate-200">Selecionar Mídia</span>
          <button
            @click="showImageModal = false"
            class="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-slate-400"
          >
